...
- **축 A — 실시간 결정성**: 추론 같은 무거운 태스크가 하드 마감시한 태스크와 공존할 때,
  마감시한을 지키는 것은 무엇인가? (답: 부하량이 아니라 **우선순위 배치와 선점 지점**)
...
그 환율이 숫자로 남았다.** 중앙값·평균·마감시한 초과 횟수는 이 차이를 하나도 못 본다 —
**분포의 꼬리만이 드러낸다.** (config당 n=5,120, 상세: `stm32-edge/measurements/action_latency.csv`)
...
명시적인 합성 부하 태스크로 분리했다. 커널 자체는 입력 데이터에 따른 변동이 0.6% 이내라
(분기 없는 구조) WCET를 한 입력의 max로 말할 수 있다.
...
<!-- TODO(W4): 작성자 검수 — 본인 Pintos 경험(과제 범위)과 어긋나는 표현이 없는지 확인 후 확정 -->
...
이 프로젝트에서 관측한 지연 증가(B-mono의 20 ms)는 **우선순위 역전이 아니라 선점**이다.
