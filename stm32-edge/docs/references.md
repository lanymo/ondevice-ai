...
| 항목 | 현재 상태 | 볼 문서 |
|---|---|---|
| **가속도/자이로 풀스케일 레인지** | **코드로 설정+되읽기 검증함** (2026-08-08, `mpu6050_configure`). 남은 것은 **비트 위치를 문서로 대조**하는 것 | RM-MPU-6000A (`ACCEL_CONFIG` 0x1C의 `AFS_SEL`, `GYRO_CONFIG` 0x1B의 `FS_SEL`) |
| MPU6050 DLPF(저역통과 필터) 설정 | **DLPF_CFG=3(≈44Hz)으로 변경함** (기본값 0 = 260Hz). 대역폭·군지연 표를 문서로 대조할 것 | RM-MPU-6000A `CONFIG`(0x1A)의 `DLPF_CFG` |
| I2C 클럭 100kHz의 근거 | 관례로 설정 | RM0383 I2C 절 + MPU6050 PS (400kHz Fast Mode 지원 여부) |

**앞의 둘은 "확인 안 했다"에서 "코드가 확인한다"로 바뀌었다.** `mpu6050_configure()`가
쓴 뒤 **되읽어 비교**하므로, 주소나 마스크가 틀리면 부팅 시 `[3b] config : FAIL`로
즉시 드러난다 — `WHO_AM_I`가 자기검증 역할을 하는 것과 같은 구조다. 다만 *되읽기가
통과해도* 확인되지 않는 게 하나 남는다: **비트 위치가 맞다는 것과, 그 값이 실제로
±2g를 뜻한다는 것은 다른 명제다.** 후자는 데이터시트의 레인지 표를 봐야 하고,
그게 위 표의 "남은 것"이다. (`MPU6050_ACCEL_LSB_PER_G` 상수가 여기에 걸려 있다.)

DLPF는 진동 감지에서 **에일리어싱**과 직결된다 — 샘플링 100Hz면 나이퀴스트 50Hz인데
센서 내부 대역폭이 그보다 넓으면 고주파가 접혀 들어온다. 하필 특징 4개 중 `mad`가
고주파 에너지 proxy라 가짜 고주파를 그대로 먹고, 접히는 위치가 진동 주파수에 따라
비단조적으로 변해 **같은 동작이 재현되지 않는다.** 대가는 군지연 ≈4.9 ms인데 윈도우가
1.28초라 0.4% — 사실상 공짜라서 바꿨다. (timeseries-windowing.md)

`SMPLRT_DIV`(0x19)는 **일부러 건드리지 않았다.** DLPF≠0이면 내부 출력레이트가 1kHz가
되는데, 이미 44Hz로 대역제한된 신호를 100Hz로 데시메이션하는 것이라 여기서 에일리어싱이
추가로 생기지 않는다. 반대로 내부 레이트를 100Hz로 "맞추면" 두 100Hz 클럭(STM32 HSI ±1%
vs MPU 내부 발진기)이 드리프트해 같은 샘플을 두 번 읽거나 하나를 건너뛰는 맥놀이가 생긴다.
**DRDY 인터럽트 없이 폴링하는 한 내부 레이트는 넉넉히 높은 편이 옳다.**
