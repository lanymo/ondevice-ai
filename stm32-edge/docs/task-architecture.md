...
| `sensor` | 40 | 384 | 10ms (`vTaskDelayUntil`) | MPU6050 폴링 100Hz + 주기 측정 |
...
[SEN] batch=0 n=256 interval_us med=10000 p99=... max=... fail=0 skip=0 accel=(...)
...
5. `accel=(x,y,z)`가 보드를 기울이면 변하는가 → 센서가 살아있다는 생존 신호.
...
- `s_win[]` (256샘플 가속도 링버퍼)에 센서 태스크가 이미 데이터를 쌓고 있다. W3의 추론
  입력 윈도우가 여기서 나온다. ([timeseries-windowing.md](timeseries-windowing.md))
